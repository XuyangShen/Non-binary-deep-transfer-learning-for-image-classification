...
def resolve_data_config(args, default_cfg={}, model=None, use_test_size=False, verbose=True):
    ...
    if args['dataset'] == 'aircraft':
        new_config['mean'] = FGVC_AIR_MEAN
    elif args['dataset'] == "dtd":
        new_config['mean'] = DTD_MEAN
    else:
    ...
    if args['dataset'] == 'aircraft':
        logging.info("Use FGVC mean&std")
        new_config['std'] = FGVC_AIR_STD
    elif args['dataset'] == "dtd":
        logging.info("Use DTD mean&std")
        new_config['std'] = DTD_STD
    else:
        logging.info("No specific dataset detects, use imagenet mean&std")

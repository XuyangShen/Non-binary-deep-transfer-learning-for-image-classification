...
class Caltech256(VisionDataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, train=True, download=False, test=False):
        ...
        for (i, c) in enumerate(self.categories):
            ...
            if train:
                self.index.extend(range(1, 31))
                self.y.extend(30 * [i])
            elif test:
                self.index.extend(range(51, n + 1))
                self.y.extend(n-50 * [i])
            else:
                self.index.extend(range(31, 51))
                self.y.extend(20*[i])

...
default_cfgs = {
    'inception_v4': {
        'url': 'https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-cadene/last.pth.tar',
        'num_classes': 1110, 'input_size': (3, 299, 299), 'pool_size': (8, 8),
...
class InceptionV4(nn.Module):
    def __init__(self, num_classes=1001, in_chans=3, output_stride=32, drop_rate=0., global_pool='avg', new_layers = 0):
...
def get_weights(m):
  for name, w in m.named_parameters():
    if 'weight' in name:
      if 'conv' in name: 
        print("weights: " + str(name))
        maxw = max(abs(w.view(-1,1)))
        print(maxw)
        w = w/10.0
        maxw = max((w.view(-1,1)))
        minw = min((w.view(-1,1)))
        w = 1e-08 + ((w - minw)/(maxw - minw))*(2.5e-02 - 1e-08)
        maxw = max(abs(w.view(-1,1)))
        minw = min(abs(w.view(-1,1)))
        print(maxw)
        print(minw)
...
def _create_inception_v4_light(variant, pretrained=False, **kwargs):
    new_layers = kwargs.get('new_layers', 0)
    ...
    if new_layers > 1:
        for i in range(1, (new_layers)):
            print(i)
            get_weights(model.features[22-i])
            """
            if i == 4:
                print('i=4')
                ...
                print('i<4')
                #print(model.features[22-i])
                
                model.features[22-i] = InceptionC() 
            """
         
    #get_weights(model.features[20])   
    return model


@register_model
def inception_v4_light(pretrained=False, **kwargs):
    return _create_inception_v4_light('inception_v4', pretrained, **kwargs)
